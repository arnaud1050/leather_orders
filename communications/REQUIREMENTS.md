...
| **F-20** | **`province` is normalised to a two-letter code** via the host's `normalize_province` — "QC", "quebec", "Québec" and "P.E.I." all resolve. | The column is two characters wide and selects the tax rate. | `test_form_mapping.py`, `test_tax.py` |
| **F-21** | An **unrecognised** province is **dropped, not stored** — "Out of country", a postal code, a sentence. Matching is exact against the folded table; no prefix or fuzzy matching. | Storing raw text truncates "Quebec" to "Qu", which matches no rule and bills the client GST-only with nothing on screen looking wrong — permanently, once an invoice is issued and frozen (see billing's freeze-at-issue). Dropping yields `no_buyer_province` ("nobody has told us"), the honest status and a prompt to fill it in, rather than `unknown_province` ("there's something on file we can't read"). Fuzzy matching is what would let "Nova Scotia office" or "not in canada" resolve to somewhere real. | `test_form_mapping.py`, `test_tax.py` |
