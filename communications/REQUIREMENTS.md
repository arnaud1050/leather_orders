...
| **R-21** | A rule's **pattern is editable in place**, and **its field mappings survive the edit**. | Changing contact-form provider, or moving off a relay onto your own domain, otherwise meant deleting the rule and retyping every label — and the mapping is the one part of this that takes real effort to get right. | `test_sender_rules.py` |
| **R-22** | An edited pattern is **cleaned and uniqueness-checked exactly like a new one**, and saving a rule unchanged is not a duplicate. | An edit can produce every mistake `add_rule` guards against. Comparing against itself would make Save — the most likely thing to press on this form — an error. | `test_sender_rules.py` |
| **R-23** | The **action is not editable**; changing it means delete and re-add. | Turning "hide everything from this sender" into "create a client from everything from this sender" is a different decision, not a correction. The two are presented as two lists because they're read as two different statements, and a dropdown that silently moved a rule between them would say nothing about what just happened. | *(structural — no control exists)* |
| **R-24** | **One label per target.** A second label pointing at a field another already fills is refused, naming the mapping in the way. **Ignore** is the one repeatable target. | The winner would be whichever appears *last in the email body* — deterministic, and unguessable from a settings page that lists mappings in the order they were added. Ignore stores nothing and exists to terminate the field above it (F-6), so a form with several skippable lines needs several. | `test_sender_rules.py` |
| **R-25** | "Full name" conflicts with "First name" / "Last name"; first and last don't conflict with each other. | Full name writes both halves, so it's the same collision one level up rather than a different question. | `test_sender_rules.py` |
| **R-26** | The conflict check applies to what's being **added**, never to what's already stored. | A deploy that quietly broke a working mapping would be a worse bug than the one it fixes. | `test_sender_rules.py` |
| **R-27** | A rule is **an address and an action** — nothing else. | It carried a free-text note that earned its keep nowhere: a tag nobody read on the list, and a second box beside the only one that matters on the edit row. Removed from the model and both forms; an existing database keeps the unused column, the same call as `lead_read_states`. | `test_sender_rules.py` |
...
| **F-19** | The address block — **street, city, province, postal code** — is mappable, on the same fill-blanks-only terms as everything else (F-8). | `Client` has had the columns since tax became destination-based; nothing could populate them automatically. Fill-blanks matters more here than elsewhere: a returning client who has **moved** keeps the address on file until a person changes it, because an unattended rule must not rewrite a billing address. | `test_form_mapping.py` |
| **F-20** | **`province` is normalised to a two-letter code** via the host's `normalize_province` — "QC", "quebec", "Québec" and "P.E.I." all resolve. | The stored value selects the tax rate, and the only values that select one are the codes. | `test_form_mapping.py`, `test_tax.py` |
| **F-21** | An **unrecognised** province is **dropped, not stored** — "Out of country", a postal code, a sentence. Matching is exact against the folded table; no prefix or fuzzy matching. | Storing raw text leaves a province that matches no row in `PROVINCE_TAXES`, so the client is charged nothing with nothing on screen looking wrong — permanently, once an invoice is issued and frozen (see billing's freeze-at-issue). (SQLite keeps the whole string despite the `VARCHAR(2)`; Postgres/MySQL would truncate or reject. Wrong either way.) Dropping yields `no_buyer_province` ("nobody has told us"), the honest status and a prompt to fill it in, rather than `unknown_province` ("there's something on file we can't read"). Fuzzy matching is what would let "Nova Scotia office" or "not in canada" resolve to somewhere real. | `test_form_mapping.py`, `test_tax.py` |
| **F-22** | `postal_code` is **uppercased**, matching `/clients/<id>/edit`. | One field, one spelling, however it arrived. | `test_form_mapping.py` |
| **F-23** | Every target the picker offers is one the mapping can actually write, and the grouped dropdown (`FIELD_TARGET_GROUPS`) covers every target exactly once. | An option that silently writes nothing is indistinguishable from a form that failed to parse; a target missing from the groups vanishes from the UI while still existing in the model. | `test_form_mapping.py` |
